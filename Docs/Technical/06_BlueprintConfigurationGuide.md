...
7. Root Preserve 是 MI 静态美术配置（Scalar 0/1，不是 Static Switch）：墙/树干模板为 `Enabled=1, Height=10cm, Feather=5cm`；屋顶与叶簇默认 0。特殊实例可在 Target 勾选 `bOverrideRootPreserve` 并填写 Height/Feather；显式 0 合法。不要给每个 foliage cluster 套根部保留。
...
11. PIE 只使用 `/Game/LostRunic/Levels/PIE_Test/L_PIE_Test`。至少验证普通移动、反复跨 Region 边缘、Local 恢复途中重新遮挡、Hard Hidden、墙/屋顶/树干/叶簇 Root 语义，以及相机贴外墙、狭窄走廊、楼梯下、高墙角、屋顶附近和 300/1400cm 特殊距离；检查无回缩、穿模背面、Near Clip 与异常 Cutaway。重点检查四个目标连续占槽、恢复值降至 `<=0.02` 后释放、第五个目标仅继续自身 CPD 而不压入描边 MPC。

本次定向自动化 `LostRunic.Cutaway` 为 4/4 通过（过渡中途反转、常量契约、请求 max 聚合与零值清理、sticky 槽优先级）；`LostRunicEditor Win64 Development` 构建通过。完整 GPU/PIE 画面验收仍以测试关卡、Lumen、VSM、Nanite 和 1080p/1440p/4K 实机观察为权威。
