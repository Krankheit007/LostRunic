...
namespace LRGuardPerceptionRules
{
	...
	LOSTRUNIC_API FLRGuardVisibilityResult EvaluateVisibility(float distance, float forwardDot,
		float sightRadius, float peripheralVisionHalfAngleDegrees,
	...
	LOSTRUNIC_API float ResolveDistanceFactor(float distance, float sightRadius,
		const FLRGuardTuningSettings& tuning);
	...
	LOSTRUNIC_API float IntegrateDetectionExposure(float currentExposureSeconds,
		const FLRGuardVisibilityResult& sample, float deltaSeconds, const FLRGuardTuningSettings& tuning);
	LOSTRUNIC_API float DecayDetectionExposure(float currentExposureSeconds, float deltaSeconds,
		const FLRGuardTuningSettings& tuning);

	LOSTRUNIC_API bool CanConfirmSight(float distance, float forwardDot,
		float sightRadius, float peripheralVisionHalfAngleDegrees, bool bOccluded, bool bHidden,
		const FLRGuardTuningSettings& tuning);
	/**
