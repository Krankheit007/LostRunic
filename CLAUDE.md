...
- **Git Diff 存档**：git diff 统一存放在仓库根目录 `GitDiff/`，文件名遵循 `git_diff_<改动名>_<提交号>_<日期>.diff`（改动名为英文 kebab-case，提交号取短哈希，日期为提交日期，如 `git_diff_core-mechanics_d467a4c_2026-08-14.diff`）；内容为纯 unified diff（无 markdown 包裹、无统计头），每个文件对应一个提交（`git diff <提交号>^ <提交号>` 生成）；不要放进 `Saved/`（生成目录）或 `Docs/Technical/`。
